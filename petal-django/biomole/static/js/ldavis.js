...
var LDAvis = function(to_select, data_or_file_name) {
    ...
    function visualize(data) {
        ...
        //-------------Kei-------------//
        /////////////////////////////////
        numTopics = data['mdsdoc'].x.length;    

        mdsDoc = [];
        for (var i = 0; i < numTopics; i++) {
            var obj = {};
            for (var key in data['mdsdoc']) {
                obj[key] = data['mdsdoc'][key][i];
            }
            mdsDoc.push(obj);
        }
        ////////////////////////////////
        ...
        // a huge matrix with 3 columns: Term, Topic, Freq, where Freq is all non-zero probabilities 
        //of topics given terms for the terms that appear in the barcharts for this data
        mdsData3 = [];
        ...
        var datKei = lamData.sort(fancysort("relevance"));   // Edit Kei
        var dat3 = datKei.slice(0, R);
        ...
        //-------------Kei-------------//
        /////////////////////////////////
        var xrange_dot = d3.extent(mdsDoc, function(d) {
            return d.x;
        }); //d3.extent returns min and max of an array
        var xdiff_dot = xrange_dot[1] - xrange_dot[0],
            xpad_dot = 0.05;
        var yrange_dot = d3.extent(mdsDoc, function(d) {
            return d.y;
        });
        var ydiff_dot = yrange_dot[1] - yrange_dot[0],
            ypad_dot = 0.05;

        if (xdiff_dot > ydiff_dot) {
            var xScale_dot = d3.scale.linear()
                    .range([0, mdswidth])
                    .domain([xrange_dot[0] - xpad_dot * xdiff_dot, xrange_dot[1] + xpad_dot * xdiff_dot]);

            var yScale_dot = d3.scale.linear()
                    .range([mdsheight, 0])
                    .domain([yrange_dot[0] - 0.5*(xdiff_dot - ydiff_dot) - ypad_dot*xdiff_dot, yrange_dot[1] + 0.5*(xdiff_dot - ydiff_dot) + ypad_dot*xdiff_dot]);
        } else {
            var xScale_dot = d3.scale.linear()
                    .range([0, mdswidth])
                    .domain([xrange_dot[0] - 0.5*(ydiff_dot - xdiff_dot) - xpad_dot*ydiff_dot, xrange_dot[1] + 0.5*(ydiff_dot - xdiff_dot) + xpad_dot*ydiff_dot]);

            var yScale_dot = d3.scale.linear()
                    .range([mdsheight, 0])
                    .domain([yrange_dot[0] - ypad_dot * ydiff_dot, yrange_dot[1] + ypad_dot * ydiff_dot]);
        }
        ////////////////////////////////
        //----------end-edit----------//
        ////////////////////////////////
        ...
        //draw dots for each abstract and add attributes
        points.append("circle")
            .attr("class", "dot")
            .style("opacity", 0.2)
            .style("fill", color1)
            .attr("r", 7)
            .attr("cx", function(d) {
                return (xScale_dot(+d.x));
            })
            .attr("cy", function(d) {
                return (yScale_dot(+d.y));
            })
            .attr("stroke", "black")
            .attr("id", function(d) {
                return (topicID + d.topics);
            ...
            .on("mouseover", function(d) {
                dot_topic_on(this);
                var x = this.__data__;
                div.transition()     // Change dot color when mouse hovers over
                    .duration(200)		
                    .style("opacity", .9);
                div.html(x.titles)	 // Show abstract titles when mouse hovers over
